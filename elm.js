...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{activeModuleTab: 4, moduleAccordionState: $rundis$elm_bootstrap$Bootstrap$Accordion$initialState, title: 'James Diffley'},
		$elm$core$Platform$Cmd$none);
};
var $author$project$Main$ModulesAccordion = function (a) {
	return {$: 'ModulesAccordion', a: a};
};
...
var $author$project$Main$subscriptions = function (model) {
	return A2($rundis$elm_bootstrap$Bootstrap$Accordion$subscriptions, model.moduleAccordionState, $author$project$Main$ModulesAccordion);
};
...
var $author$project$Data$freddos = _List_fromArray(
	[
		{inflated: 10, price: 10, year: 1994},
		{inflated: 14.3, price: 15, year: 2007},
		{inflated: 15.5, price: 17, year: 2010},
		{inflated: 16.3, price: 20, year: 2011},
		{inflated: 17.8, price: 25, year: 2014},
		{inflated: 18.9, price: 30, year: 2017},
		{inflated: 19.5, price: 25, year: 2018},
		{inflated: 20.8, price: 26, year: 2021}
	]);
...
var $terezka$elm_charts$Internal$Svg$Square = {$: 'Square'};
var $terezka$elm_charts$Chart$Attributes$square = function (config) {
	return _Utils_update(
		config,
		{
			shape: $elm$core$Maybe$Just($terezka$elm_charts$Internal$Svg$Square)
		});
};
...
var $author$project$Utils$yearToUnix = function (y) {
	return ((y - 1970) * 365.25) * (((24 * 60) * 60) * 1000);
};
var $author$project$Main$freddograph = _List_fromArray(
	[
		A2(
		...
		_List_fromArray(
			[
				...
				{bottom: 20, left: 50, right: 50, top: 10})
			]),
		_List_fromArray(
			[
				...
				A2(
					...
					$author$project$Utils$yearToUnix),
				_List_fromArray(
					[
						A3(
						...
						_List_fromArray(
							[$terezka$elm_charts$Chart$Attributes$monotone]),
						_List_fromArray(
							[$terezka$elm_charts$Chart$Attributes$square]))
					]),
				$author$project$Data$freddos)
			]))
	]);
...
var $author$project$Utils$loadStylesheet = function (path) {
	return A3(
		$elm$html$Html$node,
		'link',
		_List_fromArray(
...
var $author$project$Main$renderModuleTab = F2(
	function (model, tabnum) {
		return A2(
			$elm$html$Html$li,
			_List_fromArray(
				[
				...
				[
					...
					_List_fromArray(
						[
							...
							$author$project$Main$ChangeTab(tabnum)),
							$elm$html$Html$Attributes$classList(
							_List_fromArray(
								[
									_Utils_Tuple2(
									'active',
									_Utils_eq(model.activeModuleTab, tabnum))
								]))
						]),
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									'Year ' + $elm$core$String$fromInt(tabnum))
								]))
						]))
				]));
	});
var $elm$html$Html$ul = _VirtualDom_node('ul');
var $author$project$Main$renderModuleTabs = function (model) {
	return A2(
		$elm$html$Html$ul,
		_List_fromArray(
			[
		...
		A2(
			$elm$core$List$map,
			$author$project$Main$renderModuleTab(model),
			_List_fromArray(
				[1, 2, 3, 4])));
};
...
var $author$project$Main$moduleCard = function (m) {
	return $rundis$elm_bootstrap$Bootstrap$Accordion$card(
		{
			...
			options: _List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('modulecard')
						]))
				])
		});
};
var $author$project$Data$modules = function (year) {
	switch (year) {
		case 4:
			return _List_fromArray(
				[
					{desc: 'This module examines the general features of the A.I. problem solving process, and in particular the various forms of heuristic, together with their implementation and case studies of real systems.', title: 'Artificial Intelligence'},
					{desc: 'This module studies the design and implementation of programming languages. Topics include language design principles, abstract syntax, evaluation mechanisms, binding, type systems, polymorphism, data encapsulation, exceptions, formal definition of programming languages, compiling techniques, abstract machine design, run-time systems and garbage collection.', title: 'Programming Language Design and Implementation'},
					{desc: 'This module covers the fundamentals of signal processing and perception: investigating how sounds, images and videos can be processed and analysed alongside the fundamentals of how the human auditory and visual perception system functions (e.g., how your eyes and ears work with your brain). Concepts such as data encoding and compression are provided with practical application of understanding signals in terms of their frequency components, relating to their time and spatial components (e.g., audio frequency components or the spatial frequency of an image). Using a programming language regularly used in image and signal processing, students will gain practical skills in applying concepts to real-world problems, including using Fourier transforms, to calculate the frequency distribution in audio files, and undertake tasks such as reducing noise from signals. This module is useful for those wanting to move into the fields of computer vision or data analysis.', title: 'Signal Processing: Sound, Image, Video'},
					{desc: 'This module introduces constraint-based reasoning as a powerful mechanism for knowledge representation and inference. It provides a thorough grounding in the constraint satisfaction/constrained optimisation problem formalism, and covers both basic techniques for implementing constraint solvers and the use of advanced techniques with a modern solver.', title: 'Constraint Programming'},
					{desc: 'This module covers the fundamental concepts of computer graphics, and develops the ability to apply the concepts to the generation of realistic, synthetic images of 3D objects and scenes. On completion of the module, students should be competent to undertake many tasks in computer graphics, and should have an understanding of the theory underlying many of the relevant techniques.', title: 'Computer Graphics'},
					{desc: 'This module presents the key concepts of programming multi-core/many-core and other parallel architectures, ranging from the identification and use of parallel patterns; the use of structured parallelism to implement task and data parallelism; key implementation issues, including task identification, granularity, scheduling, threads, garbage collection, task placement, locality; performance monitoring and debugging.', title: 'Concurrency and Multi-Core Architectures'},
					{desc: 'This module builds on the general-purpose programming abilities acquired earlier, introducing games-specific techniques and material. Computer games are now a bigger industry than films, yet they are continuing to develop. While the budget for a new game may rival that of a Hollywood blockbuster, there is also a growing demand for lower octane coffee-break games that can be accessed for short periods in a browser, and for games that can be played on-the-go with a mobile device. Games programming skills are developed through lectures and laboratories, culminating in the creation of actual games.', title: 'Video Games'},
					{desc: 'This module provides an introduction to information visualisation. It focuses on the question of how to utilise visual representations to make information accessible for exploration and analysis. The module covers basic principles of visualisation design and interaction principles. It introduces a range of visualisation techniques and tools, and discusses how these can be effectively applied in various scenarios for communication, exploration and analysis, and how to evaluate information visualisations in different contexts. Skills in designing, developing, and evaluating information visualisations are reinforced through practical assignments.', title: 'Information Visualisation'}
				]);
		case 3:
			return _List_fromArray(
				[
					{desc: 'This module covers the foundations of logic that are relevant to computer scientists, with an emphasis on automatic reasoning and decidability. Topics include propositional and predicate calculus, proof techniques, the development and presentation of mathematical arguments, and Goedel’s incompleteness theorem.', title: 'Logic and Reasoning'},
					{desc: 'This module explains the techniques used to encode data, emphasising the ideas of security and secrecy, error correcting capabilities, and data compression.', title: 'Data Encoding'},
					{desc: 'This module examines the changing role of the operating system, the concept and implementation of process, the OS/hardware interface with regard to storage and protection, and the techniques developed to achieve safety and throughput in multitasking systems.', title: 'Operating Systems'},
					{desc: 'This module introduces Turing machines, non-determinism and pushdown automata, followed by study of decidability, simulation and the Halting problem. It builds upon finite state machines, context-free grammars and big-O notation from second year. The complexity classes P, NP, co-NP, NP-hard, etc., are described via analysis of SAT and graph isomorphism. Strengths and limitations of the abstract approach to complexity are discussed, followed by an in-depth introduction to practical complexity: flops, worst- and average-case analysis, approximate solutions, and case studies.', title: 'Computational Complexity'},
					{desc: 'This module introduces data models and modeling techniques, relational design and normalisation. It also examines a range of issues in database implementation, including indexing, query processing, transactions and recovery.', title: 'Databases'},
					{desc: 'This module covers the main aspects of Human Computer Interaction. Design guidelines, structured design methods and standards are studied, and practice is given in implementation and evaluation. Students gain experience of current interactive audio, visual and manipulative technologies.', title: 'Human Computer Interaction'},
					{desc: 'This module gives a broad overview of software engineering, presenting the fundamental aspects as a collaborative professional activity including its concerns and approaches. Students apply these concepts and practices to a substantial software engineering project as part of a team. Each team specifies, plans, designs, implements, tests and documents a medium-sized software system, under the guidance of a member of staff. Cooperation within and between teams is essential in order to produce successful solutions. The module provides the background and practical experience for students to enter professional careers where they will be working on large-scale software projects in teams.', title: 'Software Engineering Team Project'}
				]);
		case 2:
			return _List_fromArray(
				[
					{desc: 'This module introduces fundamental algorithms, data structures and formal language concepts at the heart of modern software, and develops skills in programming and analysis.', title: 'Foundations of Computation'},
					{desc: 'This module covers the subjects of mechanics, special relativity, oscillations, and thermal physics. It includes lectures on the dynamics of particles and rigid bodies, Einstein\'s special theory of relativity, free, forced and damped harmonic motion, and lectures on thermal physics including elementary thermodynamics and the notion of entropy.', title: 'Physics 2A'},
					{desc: 'This module extends the basic calculus in a single variable to the setting of real functions of several variables. It introduces techniques and concepts that are used throughout the mathematical sciences and physics: partial derivatives, double and triple integrals, surface sketching, cylindrical and spherical coordinates.', title: 'Multivariate Calculus'},
					{desc: 'This module develops skills in programming in C, systems programming, digital logic and low-level computer organisation.', title: 'Computer Systems'},
					{desc: 'This module introduces the functional and dynamic programming paradigms, using languages such as Haskell and Python. Understanding is reinforced through extensive practical exercises.', title: 'Advanced Programming Projects'}
				]);
		case 1:
			return _List_fromArray(
				[
					{desc: 'This module covers problem-solving skills, programming basics and object-oriented concepts, modelling and programming. Practical skills are reinforced through a range of exercises and assignments covering these topics.', title: 'Object-Oriented Programming'},
					{desc: 'This module covers the core subjects of mechanics, waves and optics, and also provides an overview of the physical properties of matter. It includes lectures on Newton\'s laws, work and energy, simple harmonic motion, the different types of wave motion, geometrical and wave optics, and the nature and composition of nuclei, atoms, molecules and solids, and their interactions. Relevant laboratory work is an important part of the module.', title: 'Physics 1A'},
					{desc: 'This module surveys our present state of knowledge of the orbits, surfaces and atmospheres of the planets in our solar system; the structure and evolution of the Sun and other stars, including extra-solar planetary systems; the bizarre menagerie of star-forming regions, violent stellar objects and supermassive black holes found within our own Milky Way Galaxy and in other galaxies; and the large-scale structure and ultimate fate of the expanding Universe. Throughout the module, fundamental observations are interpreted using mathematical models to show how distances and other properties of astronomical objects throughout the Universe have been measured, from the time of Copernicus to the era of the Hubble Telescope and beyond.', title: 'Astronomy and Astrophysics'},
					{desc: 'This module explores various aspects of data storage, processing and analysis. Programming skills are reinforced through a range of exercises and practicals covering various aspects of data handling. Topics include: persistent data formats; files and databases; file manipulation; binary and textual data; data processing using open source libraries; database design and use; object-relational mapping frameworks; processing and analysing data; issues of scale. Themes related to current research in the area of data science and big data are emphasised.', title: 'Programming with Data'},
					{desc: 'This module covers an introduction to quantum mechanics, the mechanics of rotation and gravity and an introduction to lasers. It includes lectures on the origins of quantum theory, its application to atoms and other small-scale systems; the principles of lasers, and some aspects of optical communication. The module also includes a set of group-based activities associated with the use of physics ideas to solve an interesting problem. Relevant laboratory work is an important part of the module.', title: 'Physics 1B'},
					{desc: 'This module is designed to introduce students to the ideas, methods and techniques which they will need for applying mathematics in the physical sciences or for taking the study of mathematics further. It aims to extend and enhance their skills in algebraic manipulation and in differential and integral calculus, to develop their geometric insight and their understanding of limiting processes, and to introduce them to complex numbers and matrices.', title: 'Mathematics'}
				]);
		default:
			return _List_Nil;
	}
};
var $author$project$Main$moduleCards = function (model) {
	return A2(
		$elm$core$List$map,
		$author$project$Main$moduleCard,
		$author$project$Data$modules(model.activeModuleTab));
};
...
var $author$project$Main$renderModulesAccordion = function (model) {
	return A2(
		$rundis$elm_bootstrap$Bootstrap$Accordion$view,
		model.moduleAccordionState,
		A2(
			$rundis$elm_bootstrap$Bootstrap$Accordion$cards,
			$author$project$Main$moduleCards(model),
			$rundis$elm_bootstrap$Bootstrap$Accordion$withAnimation(
				$rundis$elm_bootstrap$Bootstrap$Accordion$config($author$project$Main$ModulesAccordion))));
};
...
var $rundis$elm_bootstrap$Bootstrap$Grid$Col$xl12 = A2($rundis$elm_bootstrap$Bootstrap$Grid$Internal$width, $rundis$elm_bootstrap$Bootstrap$General$Internal$XL, $rundis$elm_bootstrap$Bootstrap$Grid$Internal$Col12);
var $author$project$Main$view = function (model) {
	return {
		body: _List_fromArray(
			[
				$rundis$elm_bootstrap$Bootstrap$CDN$stylesheet,
				$author$project$Styles$stylesheet,
				$author$project$Utils$loadStylesheet('https://fonts.googleapis.com/icon?family=Material+Icons'),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[$rundis$elm_bootstrap$Bootstrap$Grid$Col$xl12]),
								_List_fromArray(
									[
										A2(
										$elm$html$Html$div,
										_List_fromArray(
											[
												...
												$elm$html$Html$Attributes$class('baba-img')
											]),
										_List_fromArray(
											[
												A2(
												...
												_List_fromArray(
													[
														$elm$html$Html$Attributes$class('jumbo-header'),
														$elm$html$Html$Attributes$class('animate__animated'),
														$elm$html$Html$Attributes$class('animate__fadeIn'),
														$elm$html$Html$Attributes$class('animate__slower')
													]),
												_List_fromArray(
													[
														A2(
														...
														_List_fromArray(
															[
																$elm$html$Html$text('4th year computer science student (MSci)')
															])),
														A2(
														$elm$html$Html$h3,
														_List_fromArray(
														...
														_List_fromArray(
															[
																$elm$html$Html$text('University of St Andrews')
															])),
														A2(
														$elm$html$Html$a,
														_List_fromArray(
															[
																...
																$elm$html$Html$Attributes$href('#hi')
															]),
														_List_fromArray(
															[
										...
										_List_fromArray(
											[
												$elm$html$Html$Attributes$id('hi'),
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														A2($elm$html$Html$Attributes$style, 'font-size', '20rem')
													]),
												_List_fromArray(
													[
												...
												_List_fromArray(
													[
														$elm$html$Html$Attributes$class('pb-4')
													]),
												_List_fromArray(
													[
														$elm$html$Html$text(
										...
										_List_fromArray(
											[
												$elm$html$Html$Attributes$class('parallax'),
												$elm$html$Html$Attributes$class('tree-img')
											]),
										_List_Nil),
										A2(
										$elm$html$Html$div,
										_List_fromArray(
											[
												$elm$html$Html$Attributes$class('text-block'),
												$elm$html$Html$Attributes$class('px-4'),
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														$elm$html$Html$text('Here\'s what I\'ve been studying:')
													])),
												$author$project$Main$renderModuleTabs(model),
												$author$project$Main$renderModulesAccordion(model)
											])),
										A2(
										$elm$html$Html$div,
										_List_fromArray(
											[
												$elm$html$Html$Attributes$class('parallax'),
												$elm$html$Html$Attributes$class('bilb-img')
											]),
										_List_Nil),
										A2(
										$elm$html$Html$div,
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														A2($elm$html$Html$Attributes$style, 'font-size', '10rem'),
														$elm$html$Html$Attributes$class('py-4')
													]),
												_List_fromArray(
													[
														$elm$html$Html$text('work exp.')
													])),
												A2(
												$elm$html$Html$div,
												_List_fromArray(
													[
														$elm$html$Html$Attributes$id('work-idr'),
														$elm$html$Html$Attributes$class('pb-4')
													]),
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																...
																_List_fromArray(
																	[
																		$elm$html$Html$text('09/2020 -> 01/2021')
																	]))
															])),
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$href('https://www.idris-lang.org/'),
														...
														$elm$html$Html$text(' as part of a research internship at the University of St Andrews. Idris 2 is a purely-functional programming language with dependent first-class types.\r\n                                I wrote several runtime performance benchmark programs to aid our optimising of a newly-written reference-counting compiler,\r\n                                written in C. I later compiled these into a benchmarking script which can be found in the main Idris 2\r\n                                '),
														A2(
														$elm$html$Html$a,
														_List_fromArray(
															[
																$elm$html$Html$Attributes$href('https://github.com/idris-lang/Idris2/tree/main/benchmark'),
																$elm$html$Html$Attributes$target('_blank')
															]),
														_List_fromArray(
															[
												...
												_List_fromArray(
													[
														$elm$html$Html$Attributes$id('work-shrowze'),
														$elm$html$Html$Attributes$class('pb-4')
													]),
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$elm$html$Html$text('Shrowze Ltd.'),
																A2($elm$html$Html$br, _List_Nil, _List_Nil),
																A2(
																$elm$html$Html$small,
																_List_Nil,
																_List_fromArray(
																	[
																		$elm$html$Html$text('03/2020 -> 04/2020')
																	]))
															])),
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														$elm$html$Html$Attributes$id('proj-elm'),
														$elm$html$Html$Attributes$class('pb-4')
													]),
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$href('https://github.com/jmd28/elm-site'),
																$elm$html$Html$Attributes$target('_blank')
															]),
														_List_fromArray(
															[
												...
												_List_fromArray(
													[
														$elm$html$Html$Attributes$id('proj-disc'),
														$elm$html$Html$Attributes$class('pb-4')
													]),
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$href('https://github.com/jmd28/Ewan2.0'),
																$elm$html$Html$Attributes$target('_blank')
															]),
														_List_fromArray(
															[
											...
											[
												...
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$href('https://www.facebook.com/BigBandoftheUniversityofStAndrews'),
																$elm$html$Html$Attributes$target('_blank')
															]),
														_List_fromArray(
															[
																$elm$html$Html$text('(Big BUStA)')
															])),
														$elm$html$Html$text(' and have appeared in numerous smaller bands, combos and orchestras over the years - which has taken me touring all across Europe.\r\n                                During the lockdowns of 2020 and 2021, I worked on producing the online video performances for Big BUStA, which involved\r\n                                mixing together individual mobile recordings of the players using a DAW (Reaper) and then editing together the video using Adobe Premiere Pro.\r\n                                ')
													])),
												A2(
												$elm$html$Html$div,
												_List_fromArray(
												...
												_List_fromArray(
													[
														$elm$html$Html$text('look at this graph')
													])),
												$elm$html$Html$text('You can have five arbitrary points if you can guess what it shows'),
												A2(
												$elm$html$Html$div,
												_List_fromArray(
													[
												...
												$author$project$Main$freddograph)
											])),
										A2(
										$elm$html$Html$div,
										_List_fromArray(
											...
											[
												...
												_List_fromArray(
													[
														...
														_List_fromArray(
															[
																$elm$html$Html$Attributes$class('link-div'),
																$elm$html$Html$Attributes$href('mailto:jmd28@st-andrews.ac.uk')
															]),
														_List_fromArray(
															[
																A2(
																...
																$elm$html$Html$text('jmd28@st-andrews.ac.uk')
															])),
														A2(
														$elm$html$Html$a,
														_List_fromArray(
															[
																$elm$html$Html$Attributes$class('link-div'),
																$elm$html$Html$Attributes$href('https://github.com/jmd28'),
																$elm$html$Html$Attributes$target('_blank')
															]),
														_List_fromArray(
															[
